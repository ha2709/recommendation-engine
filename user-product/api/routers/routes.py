...
def get_all_users(
    api_key: str = Depends(get_api_key), session: Session = Depends(get_session)
):
...
def read_transaction(
    ...
    if not transaction:
        message = f"Transaction with id {transaction_id} not found not found"
        logging.error(message)
        raise HTTPException(status_code=404, detail=message)
    return transaction
...
@router_v1.put("/transaction/{transaction_id}", response_model=TransactionSchema)
...
@router_v1.delete(
    "/transaction/{transaction_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
# Create a Product
@router_v1.post(
    "/product", response_model=ProductSchema, status_code=status.HTTP_201_CREATED
)
def create_product(
